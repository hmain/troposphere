...
def main():
    ...
    t.add_description("Template for auto-scaling windows instances bootstrapped with a powershell script in "
                      "the instance launchconfiguration userdata. The instances ALB will be used as "
                      "an A Alias target for a Route53 hosted zone. "
    ...
    EC2LaunchConfiguration = t.add_resource(autoscaling.LaunchConfiguration(
        ...
        UserData=Base64(Join('\n', [
            "<powershell>",
            # Start logging
            "start-transcript -append -path c:/Temp/cloudInit.log",
            "write-output 'start'",
            "get-date -f FileDateTime 2>&1 | Write-Output",
            # Install AWS cli
            "(new-object net.webclient).DownloadFile('https://s3.amazonaws.com/aws-cli/AWSCLI64.msi', 'aws-cli.msi') 2>&1 | Write-Output",
            "msiexec /i aws-cli.msi /quiet 2>&1 | Write-Output",
            "Import-Module AWSPowerShell 2>&1 | Write-Output",
            # Install IIS
            "Install-WindowsFeature -Name 'Web-Server' 2>&1 | Write-Output",
            "Get-WindowsFeature -Name Web-*45* | Install-WindowsFeature 2>&1 | Write-Output",
            "Get-WindowsFeature -Name NET-Framework-45-* | Install-WindowsFeature 2>&1 | Write-Output",
            # Stop logging
            "get-date -f FileDateTime 2>&1 | Write-Output",
            "write-output 'end'",
            "stop-transcript",
